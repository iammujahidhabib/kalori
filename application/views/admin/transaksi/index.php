                            ...
                            <tbody>
                                ...
                                foreach ($transaksi as $key) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-secondary btn-sm text-white" id="btn_lihat" data-id="<?= $key->id_transaksi ?>" data-nama="<?= $key->nama ?>" data-date="<?= $key->date ?>" data-nama_customer="<?= $key->nama_customer ?>" data-tanggal_pesan="<?= $key->tanggal_pesan ?>" data-qty="<?= $key->qty ?>" data-total="<?= $key->total ?>" data-bukti="<?= $key->bukti ?>" data-nomor_penerima="<?= $key->nomor_penerima ?>" data-alamat_kirim="<?= $key->alamat_kirim ?>" data-durasi="<?= $key->durasi ?>" data-gram="<?= $key->gram ?>" data-sayur="<?= $key->sayur ?>" data-kalori="<?= $key->kalori ?>" data-batas="<?= $key->batas ?>" data-status_transaksi="<?php if ($key->status_transaksi == 0) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    echo "Menunggu Bukti Pembayaran";
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                } elseif ($key->status_transaksi == 1) {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    echo "Menunggu Validasi Pembayaran oleh Katering";
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                } elseif ($key->status_transaksi == 2) {
                                            ...
                                            <?php } ?>
                                            <?php if ($key->status_transaksi == 3) { ?>
                                                <a class="btn btn-outline-secondary btn-sm" id="btn_jadwal" data-id="<?= $key->id_transaksi ?>">Jadwal</a>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php $no++;
...
<div class="modal fade" id="jadwalModal" tabindex="-1" aria-labelledby="jadwalModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" style="max-width: 90vw !important;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="jadwalModalLabel">Detail Jadwal Pesanan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', "#btn_jadwal", function() {
            event.preventDefault()
            $.ajax({
                type: "GET",
                url: "<?= site_url('cms/vendor/jadwal_json') ?>/" + $(this).data("id"),
                success: function(data) {
                    $("#body_jadwal").html(data);
